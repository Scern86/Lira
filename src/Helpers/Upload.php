...
{	
    ...
    {		
		$request = \Scern\Lira\Core\Request::getInstance();
		$config = \Scern\Lira\Core\Config::getInstance();		
		if ($request->post('action')=='upload' AND !empty($_FILES['upload']['name'][0])) {
			...
			for ($a=0;$a<$total;$a++) {
				...
				if ($old_file !='') {
					...
					if (move_uploaded_file($old_file,$new_file)) {
						...
						$id_object = \Scern\Lira\Helpers\Generator::generateId(10);
						$data = [
							'id' => $id_object,
							'title' => htmlspecialchars(str_replace('.'.pathinfo($old_filename)['extension'],'',$old_filename)),
							'created' => date('Y-m-d H:i:s'),
						];
						$object = \Scern\Lira\Classes\Object::create($data);
						// Если это изображение
						if(in_array(strtolower(pathinfo($old_filename)['extension']), $image_types)){
							\Scern\Lira\ClassesObject::addField($id_object,'type',[\Scern\Lira\Classes\Tag::getIdByTitle('Изображение')],0);
							...
							if($request->post('resize_images')==1){
								...
								\Scern\Lira\Classes\Object::addField($id_object,'small',$hostname.DS.$path.DS.'small_'.$new_image_name);
								\Scern\Lira\Classes\Object::addField($id_object,'medium',$hostname.DS.$path.DS.'medium_'.$new_image_name);
								\Scern\Lira\Classes\Object::addField($id_object,'large',$hostname.DS.$path.DS.$new_filename);								
							}
							else{
								\Scern\Lira\Classes\Object::addField($id_object,'small',$hostname.DS.$path.DS.$new_filename);
								\Scern\Lira\Classes\Object::addField($id_object,'medium',$hostname.DS.$path.DS.$new_filename);
								\Scern\Lira\Classes\Object::addField($id_object,'large',$hostname.DS.$path.DS.$new_filename);
							}
						}
						//Если это документ
						if(in_array(strtolower(pathinfo($old_filename)['extension']),$document_types)){
							\Scern\Lira\Classes\Object::addField($id_object,'type',[\Scern\Lira\Classes\Tag::getIdByTitle('Документ')],0);
							\Scern\Lira\Classes\Object::addField($id_object,'path',$hostname.DS.$path.DS.$new_filename);
						}
						\Scern\Lira\Classes\Object::addField($id_object,'access',['5e1cc7832dcbd43148d5'],0);
			...
			if(count($result['success']>0)) {
				$result['done'] = TRUE;
				return $result;
			} else \Scern\Lira\Core\Document::addContent('content', 20, \Scern\Lira\Helpers\Render::render('templates'.DS.'shared', 'upload', ['multiple' => $multiple, 'resize_images' => $resize_images]));
		} else {
			\Scern\Lira\Core\Document::addContent('content',20,\Scern\Lira\Helpers\Render::render('templates'.DS.'shared', 'upload', ['multiple' => $multiple, 'resize_images' => $resize_images]));
			return FALSE;			
		}
	}
