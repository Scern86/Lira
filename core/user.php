...
class UserCore{
	...
	public function checkLogin(){		
		if($this->_session->logged_in) {
			$id_user = $this->_db->getOne('SELECT l.`id_user` FROM ?n l,?n t WHERE l.`id_user`=t.`id_object` AND t.`definition`=?s AND t.`id_tag`=?s AND l.`ssid`=?s AND DATE(l.`logged_in`)=?s AND l.`ip_address`=?s',TBL_LOGIN,TBL_EXT_TAG,'type','86b5e76b8a6637060559',$this->_session->getSessionId(),date('Y-m-d'),$_SERVER['HTTP_X_REAL_IP']);
			if(!empty($id_user)){
 	...
 	public function verifyUser($login,$password){
		if(!empty($login) AND !empty($password)){
			...
			if(!empty($id_user)){
				$this->_db->query('INSERT INTO ?n SET `id_user`=?s,`ssid`=?s,`ip_address`=?s',TBL_LOGIN,$id_user,$this->_session->getSessionId(),$_SERVER['HTTP_X_REAL_IP']);
				$this->_user = new ObjectClasses($id_user);
				$this->logged_in = TRUE;
